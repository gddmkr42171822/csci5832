...
def checkCommandLineArgs(): 
    ...
    if len(sys.argv) < 4:
        print("Usage: werthman-assgn1.py <absolute path to file of word list> <absolute path to file of hashtags> <word reference for WER>")
        sys.exit()
    else:
        #check if the user gave the absolute path for both files otherwise print script usage and exit
        if os.path.isabs(sys.argv[1]) == False or os.path.isabs(sys.argv[2]) == False or os.path.isabs(sys.argv[2]) == False:
            print("Usage: werthman-assgn1.py <absolute path to file of word list> <absolute path to file of hashtags> <word reference for WER>")
            sys.exit()
        else:
...
def runMaxMatchMinEdit(maxmatchFunction, wordlist, hashtaglist, referenceOutputPath):
    ...
    print("Writing the maxmatched hashtags to the file werthman-out-assgn1.txt")
    with open('werthman_out_assgn1.txt', 'w') as f:
    ...
    maxmatchHashtags = []
    with open('werthman_out_assgn1.txt', 'r') as f:
        for line in f:
            maxmatchHashtags.append(line.strip())
            
    #load correct hashtags into a set
    correctHashtags = []
    with open(referenceOutputPath, 'r') as f:
        for line in f:
...
def main():
    ...
    (wordlistPath, hashtaglistPath, referenceOutputPath) = checkCommandLineArgs()
    print("Path to the word list: " + wordlistPath)
    print("Path to the hashtag list: " + hashtaglistPath)
    print("Path to the reference output: " + referenceOutputPath)
    ...
    runMaxMatchMinEdit(maxMatch, wordlist, hashtaglist, referenceOutputPath)
    print("----Running backwards maxmatch function----")
    runMaxMatchMinEdit(backwardsMaxMatch, wordlist, hashtaglist, referenceOutputPath)
    print("----Running any maxmatch function----")
    runMaxMatchMinEdit(anyMaxMatch, wordlist, hashtaglist, referenceOutputPath)
